...
public class BinaryTreeIterable<T extends Comparable<? super T>> implements Iterable {
    ...
    private void imprimeLargura(Node<T> raiz) {
    ...
    public void removeFolhas() {
        raiz = removeFolhas(raiz);
    }

    private Node<T> removeFolhas(Node<T> node) {
        if (node == null)
            return null;
        ...
        if (node.getLeft() != null) {
            node.setLeft(removeFolhas(node.getLeft()));
        }

        if (node.getRight() != null) {
            node.setRight(removeFolhas(node.getRight()));
        }

        return node;
    }

    public Node sucessor(T value) {
        return sucessor(raiz, value);
    }

    private Node sucessor(Node<T> no, T value) {
        if (no == null) {
            return no;
        }

        if (value.compareTo(no.getNode()) < 0) {
            Node sucessorMenor = sucessor(no.getLeft(), value);
