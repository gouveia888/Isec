function [retrieved_indexes, similarities, new_case] = retrieve(case_library, new_case, threshold)
    ...
    weighting_factors = [5 10 7 9 2 6 9 8]; %peso atributo tipo de ferias, preço, numero de pessoas, regiao etc

    %criar tabelas de similariadades
    ...
    %valor para usar na normalizaçao
    ...
    for i=1:size(case_library,1)
        ...
        distances = zeros(1,8); %calculo de 8 distancias locais pelaa ordem que esta no cbr.m
        ...
        Dglobal = (distances * weighting_factors')/sum(weighting_factors);
        final_similarity = 1 - Dglobal;
...
function [holiday_type_sim] = get_holiday_type_similarities()
    ...
    holiday_type_sim.similarities = [
        % Active Bathing City Education Language Recreation Skiing Wandering
           1.0     0.4   0.8    0.2        0.3       0.7      0.8     0.1    % Active
           ...
           0.8     0.8   0.6    0.6        0.2       1.0      0.1     0.7    % Recreation
           0.7     0.7   0.2    0.2        0.0       0.1      1.0     0.7    % Skiing
           0.8     0.5   0.7    0.6        0.4       0.7      0.7     1.0    % Wandering
    ];
end
...
function [res] = calculate_local_distance(sim, val1, val2)
    ...
    res = 1-sim.similarities(i2,i2);   %converte semelhança para distancias
end
...
function [res] = calculate_haversine_distance(latlon1, latlon2)
    ...
    res = distance/40075; %normalizaçao pelo diametro descrito sobre a terra
end
