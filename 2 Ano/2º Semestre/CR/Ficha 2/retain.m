function [case_library] = retain(case_library, new_case, journey_index)
    ...
    lista = {'holiday_type', 'price', 'number_persons', 'region', 'transportation', 'duration','season', 'accommodation'};
    ...
    for i=1:length(lista)
        if ~isfield(new_case, lista{i})
          new_case = setfield(new_case, lista{i}, case_library{journey_index, i+1}) %a primeira coluna é o id
        end
    end
