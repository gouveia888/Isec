...
DWORD WINAPI MovimentaImagem(LPVOID lparam) {
    ...
    while (1) {
        ...
        InvalidateRect(dados->hWnd, NULL, TRUE); //wmpaint
        Sleep(30);
    }
}
...
LRESULT CALLBACK trataEventos_a(HWND, UINT, WPARAM, LPARAM);
LRESULT CALLBACK trataEventos_b(HWND, UINT, WPARAM, LPARAM);
LRESULT CALLBACK trataEventos_c(HWND, UINT, WPARAM, LPARAM);
...
// ============================================================================
    ...
    wcApp.lpfnWndProc = trataEventos_a;  // Endereço da função callback de processamento da janela
    // ("TrataEventos" foi declarada no início e
    // encontra-se mais abaixo)
    wcApp.style = CS_HREDRAW | CS_VREDRAW;  // Estilo da janela: Fazer o redraw se for
    // modificada horizontal ou verticalmente
    wcApp.hIcon = LoadIcon(NULL, IDI_SHIELD);  // "hIcon" - handler do ícone normal
...
// ============================================================================
    ...
    hWnd = CreateWindow(
        ...
        TEXT("Exercicio 6"),  // Texto que figura na barra do título
...
// 1) &lpMsg  - Endereço de uma estrutura do tipo MSG ("MSG lpMsg" foi declarada no  
//              início de _tWinMain()):
...
LRESULT CALLBACK trataEventos_a(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        hBmp = (HBITMAP)LoadImage(NULL,
            TEXT("ballon.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        GetObject(hBmp, sizeof(bmp), &bmp);  // -> obtem informação acerca do objeto apontado pelo handle (neste caso, o tamanho)
        hdc = GetDC(hWnd);
    ...
    case WM_GETMINMAXINFO: //Permite fixar tamanhos mínimo/máximo da janela quando o utilizador a redimensiona.
        mmi = (MINMAXINFO*)lParam;
        mmi->ptMinTrackSize.x = bmp.bmWidth + 2;
        mmi->ptMinTrackSize.y = bmp.bmHeight + 24 + 2;
        break;

    case WM_SIZE: //Actualiza as coordenadas da imagem
