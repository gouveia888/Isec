...
LRESULT CALLBACK trataEventos_b(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam);
...
// ============================================================================
    ...
    wcApp.lpfnWndProc = trataEventos_b;  // Endereço da função callback de processamento da janela
...
LRESULT CALLBACK trataEventos_a(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DESTROY:
        ...
        break;
    case WM_CLOSE:
        if (MessageBox(hWnd, _T("Deseja fechar a janela?"), _T("Confirmação"), MB_ICONQUESTION | MB_YESNO) == IDYES)
            DestroyWindow(hWnd);
        break;
    case WM_CREATE:
        hBmp = (HBITMAP)LoadImage(NULL, _T("ballon.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
...
LRESULT CALLBACK trataEventos_b(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (messg) {
    case WM_CREATE:
        hBmp = (HBITMAP)LoadImage(NULL, TEXT("ballon.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
        GetObject(hBmp, sizeof(bmp), &bmp);
