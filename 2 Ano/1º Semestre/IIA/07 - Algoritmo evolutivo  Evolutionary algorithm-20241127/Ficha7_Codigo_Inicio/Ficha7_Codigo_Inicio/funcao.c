...
void evaluate(pchrom pop, struct info d, int mat[][2])
{
	...
	for (i=0; i<d.popsize; i++)
		pop[i].fitness = eval_individual2(pop[i].p, d, mat, &pop[i].valido);
...
{
    int     i;
    float   sum_weight, sum_profit, ro_aux, penalty;
    d.ro = -1;
    ...
    if (sum_weight > d.capacity)
    {
        // Solucao inválida
        *v = 0;
        penalty = d.ro * (sum_weight - d.capacity);
    }
    else
    {
        // Solucao válida
