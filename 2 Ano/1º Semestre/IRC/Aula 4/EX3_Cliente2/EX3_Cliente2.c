...
int main(int argc, char* argv[])
{
	...
	int nbytes = recvfrom(sockfd, msg_rec, sizeof(msg_rec), 0, NULL, NULL);

	//EX8
	if (nbytes == SOCKET_ERROR) {
		if (WSAGetLastError() != WSAETIMEDOUT) Abort("Error enquanto aguarda pela resposta");
		else Abort("Timeout de recepcao");
	}


	if (nbytes < BUFFERSIZE) {
		msg_rec[nbytes] = '\0';
		printf("<CLI-2> Mensagem do Servidor: %s\n", msg_rec);
	}
	else printf("<CLI-2> Mensagem do Servidor %d ecede o valor previsto %d\n", nbytes, BUFFERSIZE);
