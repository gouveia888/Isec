...
void AtendeCliente(LPVOID param){
	...
	if ((sockDest = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == SOCKET_ERROR){
	...
	while(1){
		...
		switch(select(0, &fdtemp, NULL, NULL, NULL)){ // Sem timeout
			case SOCKET_ERROR:
...
int exchange(SOCKET s1, SOCKET s2)
{
	...
	if ((result = recv(s1, &c, sizeof(c), 0)) == sizeof(c))
		send(s2, &c, result, 0);
