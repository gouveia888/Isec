...
int main( int argc , char *argv[] )
{
	SOCKET sockfd;
	int iResult, nbytes;
	struct sockaddr_in serv_addr; //recebe dados do cliente
	...
	while(1){
		...
		nbytes=recvfrom(
			...
			0,									//Flags
			&cli_addr,							//Quem esta a enviar a mensagem
			&cli_addr_size						//Size of quem envia mensagem
		);
		...
		sprintf_s(msg, sizeof(msg), "%d", (int)strlen(buffer));
		...
		//Enviar a mensagem de retorno
		// ATE AO EX10
		/*if (sendto(sockfd, msg , strlen(msg) + 1, 0, (struct sockaddr*)&cli_addr, sizeof(cli_addr)) == SOCKET_ERROR)
