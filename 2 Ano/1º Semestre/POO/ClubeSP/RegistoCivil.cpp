...
void RegistoCivil::redimensionarPessoas()
{
    ...
    {
        //copiar os shared pointers para pessoas para o novo array
...
bool RegistoCivil::removePessoa(int BI)
{
    for (int i = 0; i < nPessoas; i++)
    {
        ...
        {
            int j;
            for(j = i; j < nPessoas-1; j++) // nPessoas -1 para nao acedermos fora do array
                pessoas[j] = pessoas[j+1];
            pessoas[nPessoas-1].reset(); //        pessoas[nPessoas-1].reset();
            --nPessoas;
            return true;
        }
    }
...
{
    ostringstream oss;
    oss << "Registo Civil de: " << pais << endl << "N Pessoas:" << nPessoas << endl;
    for (int i = 0; i < nPessoas; i++)
